...
export const ProductServices = (props) => {
    ...
    const handleImageInputChange = (e) => {
        ...
        (async () => {
            try {
                setCreateServicesProps(prevState => ({...prevState, iconText: e.target.files[0]?.name || ''}));
                const base64String = await getBase64(e.target.files[0]);
                setIconString(base64String)
            } catch (error) {
                console.log(error);
            }
        })()
    ...
    const onFinish = async (values) => {
        try {
            ...
            if(!iconString){
                notificationAlert('error', 'Invalid Icon', 'Please upload a service icons');
            ...
            setCreateServicesProps(prevState => ({...prevState, loading: false, visible: false}))
            notificationAlert('success', 'Service Created', `${values.name} service has been created`);
        } catch (error) {
            setCreateServicesProps(prevState => ({...prevState, loading: false}))
            notificationAlert('error', 'Error Occurred', error.message || 'Please try again later');
        }
    ...
    return (
        <>
            <PageTitleBar onButtonClick={() => setCreateServicesProps(prevState => ({...prevState, visible: true}))} buttonText='Create Service' hideButtons={false} title="Services" />
            <TableComponent >
                <AntTable
                    columns={columns}
                    dataSource={services}
            ...
            <CreateServicesModal
                ...
                onCancel={() => setCreateServicesProps(prevState => ({...prevState, visible: false}))}
        ...
        <StyledModal
            ...
            >
                ...
                <Form.Item label='Description' name='description'  rules={[{required: true,}]}>
                    <StyledInputContainer>
                        <Input.TextArea required autoSize={{ minRows: 2, maxRows: 6 }} bordered={false} />
                    </StyledInputContainer>
                </Form.Item>
                <Form.Item  name='iconBase64String'>
                    <div>
                        <PrimaryButton type='dashed' onClick={handleImageInputClick} text='Upload Icon'/>
                        <input onChange={props.handleImageInputChange} ref={imageInputRef} type='file' style={{display: 'none'}}/>
                        <p>{props.iconText}</p>
                    </div>
                </Form.Item>
            </Form>
